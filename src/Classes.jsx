...
const styles = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .class-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); transition: background 0.3s, border-color 0.3s, transform 0.3s; display: flex; flex-direction: column; overflow: hidden; }
  ...
  .class-card-img { width: 100%; height: 100%; object-fit: cover; object-position: center; display: block; transition: transform 0.5s ease; }
  .class-card:hover .class-card-img { transform: scale(1.05); }
  .class-card-img-placeholder { width: 100%; height: 100%; background: linear-gradient(135deg, #1a2535, #0d1525); display: flex; align-items: center; justify-content: center; font-size: 52px; opacity: 0.25; }

  .class-card-body { padding: 28px; display: flex; flex-direction: column; flex: 1; }
  .class-card-title { font-family: 'Bebas Neue', cursive; font-size: 28px; letter-spacing: 1px; color: var(--white); margin-bottom: 16px; line-height: 1; }
  .class-meta { display: flex; flex-direction: column; gap: 8px; margin-bottom: 20px; flex: 1; }
  ...
  .class-spots { font-family: 'Barlow Condensed', sans-serif; font-size: 14px; font-weight: 700; letter-spacing: 1px; color: var(--gold); margin-bottom: 20px; display: flex; align-items: center; gap: 8px; }
  .spots-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--gold); flex-shrink: 0; }
  .btn-book { background: var(--white); color: var(--darker); border: none; padding: 14px; font-family: 'Barlow Condensed', sans-serif; font-size: 12px; letter-spacing: 4px; font-weight: 700; text-transform: uppercase; cursor: pointer; transition: all 0.25s; width: 100%; margin-top: auto; }
  ...
  .section-title { font-family: 'Bebas Neue', cursive; font-size: clamp(40px, 5vw, 64px); line-height: 1; color: var(--white); margin-bottom: 48px; }
...
export default function Classes() {
  ...
  const headerBg = content.classesPage.classList.find(c => c.image)?.image || null
          ...
          {content.classesPage.classList.map((cls, i) => (
            <div key={i} className="class-card">
              <div className="class-card-img-wrap">
                {cls.image
                  ? <img src={cls.image} alt={cls.name} className="class-card-img" />
                  : <div className="class-card-img-placeholder">🏋️</div>
                }
              </div>
