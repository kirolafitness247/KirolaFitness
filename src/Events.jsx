    ...
    <>
      ...
      <section className="section" style={{background: 'var(--darker)'}}>
        <div className="grid-3">
          {content.eventsPage.events.map((e, i) => (
            <div key={i} className="card">
              <div style={{display:'inline-block',padding:'4px 12px',background:'rgba(201,168,76,0.2)',color:'var(--gold)',fontSize:'10px',letterSpacing:'2px',marginBottom:'12px',textTransform:'uppercase',fontWeight:'700'}}>{e.cat}</div>
              <h3 className="card-title">{e.title}</h3>
              <div style={{fontSize:'13px',color:'var(--gold)',marginBottom:'12px'}}>📅 {e.date}</div>
              <p className="card-desc" style={{marginBottom:'20px'}}>{e.desc}</p>
